...
function Badge({ fmt, type }: { fmt: FormatOption; type: "video" | "audio" }) {
  ...
  return (
    <span
      className={`text-[9px] font-bold px-2 py-0.5 rounded border uppercase tracking-widest ml-2 ${
        isMerged && type === "video"
...
function FormatRow({
  ...
  return (
    ...
    >
      {index === 0 && (
        <span className="absolute -left-1 top-4 bottom-4 w-1 bg-yt-red rounded-r-full" />
      )}
      {/* Left */}
  ...
  const handleDownload = async (fmt: FormatOption) => {
    ...
    try {
      const response = await fetch(
        `/api/download?videoId=${encodeURIComponent(data.videoId)}&format_id=${encodeURIComponent(formatId)}`,
      );
      ...
      alert(
        "Proxy download failed. This can happen with very large files in a browser memory.",
      );
    } finally {
            ...
            >
              <div className="rounded-xl border border-white/6 bg-white/2 p-4">
                ...
                <p className="text-xs text-white/45 leading-relaxed">
                  Merged streams (up to 720p) include audio. For 1080p+, YouTube
                  requires downloading 'Video Only' and 'Audio' separately.
                </p>
              </div>
            </motion.div>
          </div>
